    ...
    : QWidget(parent, f), _layout(this)
{
    ...
    connect(listchart, &QP_ListChart::sigSelectPart, this, &QP_DiskView::slotListChartSelectPart);
    connect(listchart, &QP_ListChart::sigPopup, this, &QP_DiskView::sigPopup);
    connect(listchart, &QP_ListChart::sigDevicePopup, this, &QP_DiskView::sigDevicePopup);
    _layout.addWidget(listchart);

    /*---create the listview, attach it and connect the signals---*/
    listview = new QP_ListView(this);
    connect(listview, &QP_ListView::sigSelectPart, this, &QP_DiskView::slotListViewSelectPart);
    connect(listview, &QP_ListView::sigPopup, this, &QP_DiskView::sigPopup);
    ...
    connect(libparted, &QP_LibParted::sigTimer, this, &QP_DiskView::sigTimer);

    /*---connect the signal "update progressbar" during commit!!---*/
    connect(libparted, &QP_LibParted::sigOperations, this, &QP_DiskView::sigOperations);

    /*---connect the signal to get if the disk changed (used for undo/commit)---*/
    connect(libparted, &QP_LibParted::sigDiskChanged, this, &QP_DiskView::sigDiskChanged);
}
...
void QP_DiskView::refresh_widgets()
{
    ...
    for (QP_PartInfo* p : libparted->partlist)
    {
        addPrimary(p);

        if (p->type == QTParted::extended)
        {
            for (QP_PartInfo* logi : libparted->logilist)
            {
