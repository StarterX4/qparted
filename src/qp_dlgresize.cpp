...
QP_dlgResize::QP_dlgResize(QWidget *parent) : QDialog(parent), Ui::QP_UIResize() {
	...
	sizecontainer = nullptr;

	/*---this is a fixed dialog!---*/
	setFixedSize(minimumSizeHint());

	sizecontainer = new QP_SizeContainer(container);
	connect(sizecontainer, &QP_SizeContainer::sigChangedStart,
			this, &QP_dlgResize::slotChangedStart);
	connect(sizecontainer, &QP_SizeContainer::sigChangedEnd,
			this, &QP_dlgResize::slotChangedEnd);
	connect(sizecontainer, &QP_SizeContainer::sigChangedPos,
			this, &QP_dlgResize::slotChangedPos);

	connect(spinFreeBefore, QOverload<int>::of(&QSpinBox::valueChanged),
			this, &QP_dlgResize::slotFreeBeforeChanged);
	connect(spinFreeAfter, QOverload<int>::of(&QSpinBox::valueChanged),
			this, &QP_dlgResize::slotFreeAfterChanged);
	connect(spinNewSize, QOverload<int>::of(&QSpinBox::valueChanged),
			this, &QP_dlgResize::slotNewSizeChanged);
}
...
void QP_dlgResize::setValFreeBefore() {
    ...
    spinFreeBefore->setMaximum(maxfree);
...
void QP_dlgResize::setValFreeAfter() {
    ...
    spinFreeAfter->setMaximum(maxfree);
