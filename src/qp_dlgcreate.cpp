...
QP_dlgCreate::QP_dlgCreate(QWidget *w):QDialog(w),Ui::QP_UICreate() {
    setupUi(this);

    /*---connect the combo type slot---*/
    connect(cmbType, QOverload<int>::of(&QComboBox::activated),
            this, &QP_dlgCreate::slotTypeChanged);

    /*---connect the combo filesystem type slot---*/
    connect(cmbFilesystem, QOverload<int>::of(&QComboBox::activated),
            this, &QP_dlgCreate::slotFSChanged);

    /*---connect the spinratio slot (received when you change the size)---*/
    connect(spinRatio, QOverload<int>::of(&QSpinBox::valueChanged),
            this, &QP_dlgCreate::slotRatioChanged);

    /*---connect the spinsize slot (received when you change the size)---*/
    connect(spinSize, QOverload<PedSector>::of(&QP_ComboSpin::valueChanged),
            this, &QP_dlgCreate::slotSizeChanged);
}
...
void QP_dlgCreate::slotFSChanged(int) {
    ...
    if (type() != QTParted::extended) {
        ...
        if (spinSize->maximum() != newmaxsize) {
            spinSize->setMaximum(newmaxsize);
        }

        spinSize->setMinimum(fsspec->minFsSize());
    }
}
...
void QP_dlgCreate::slotRatioChanged(int value) {
    ...
    int newsize = (spinSize->maximum() * value) / 100;
...
void QP_dlgCreate::slotSizeChanged(PedSector value) {
    ...
    int newratio = (value * 100) / spinSize->maximum();
